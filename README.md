...
Про качество честно. «Где происходит проверка сертификата сервера» даёт точное
попадание первым результатом. «Разбор параметров командной строки» поднимает
`getopt` и `argparse` в топ-4, но первым может встать похожий по смыслу, но не тот
модуль. Потолок здесь — дефолтная модель со 128-токенным окном: она видит имя,
докстринг и описание модуля, но не тело функции. Нужна точность выше — берите
`multilingual-e5-large` или `voyage-code-3`, конвейер от этого не меняется.
...
```bash
pytest                    # 34 теста, идут на провайдере hash — сеть не нужна
cd examples/demo_repo && fyc index && fyc find "проверка пароля"
```

## Что дальше

- [ ] `fyc watch` — дообновление индекса по файловым событиям
